...
def evaluate_config(pipe, prompts, cfg, clip_model, preprocess):
    ...
    for p in prompts:
        ...
        metrics[f"{cfg['name']}_clip_score"] = clip_s
        metrics[f"{cfg['name']}_warp_error"] = compute_warp_error(tens)
        metrics[f"{cfg['name']}_brisque_error"] = compute_brisque_score(tens)
        metrics[f"{cfg['name']}_runtime"] = rt
...
def run_evaluation(out="eval_outputs"):
    ...
    with mlflow.start_run(run_name=datetime.now().strftime("eval_%Y%m%d_%H%M%S")):
        # log_all_system_metrics()  # ⬅️ moved inside run
        pipe = load_pipeline()
...
def evaluate_local_video(video_path: str, prompt: str, clip_model, preprocess, height: int = None, width: int = None):
    with mlflow.start_run(run_name="local_video_eval_" + datetime.now().strftime("%Y%m%d_%H%M%S")):
        # log_all_system_metrics()  # ⬅️ inside run context
        log_model_info(clip_model, model_name="CLIP ViT-B/32", device=DEVICE)
        ...
        metrics[f"clip_score"] = clip_s
        metrics[f"warp_error"] = compute_warp_error(tens)
        metrics[f"brisque_error"] = compute_brisque_score(tens)
        metrics[f"runtime"] = float('nan')
        metrics["clip_mean"] = float(np.mean([v for k, v in metrics.items() if k.endswith("_clip")]))
